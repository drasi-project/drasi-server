...
fn create_test_plugin_registry() -> PluginRegistry {
    ...
    if plugin_dir.exists() {
        ...
        match stats {
            ...
            Err(e) => {
                eprintln!("WARNING: Failed to load test plugins: {e}");
            }
        }
    } else {
        eprintln!(
...
pub async fn create_test_router_with_solutions(
    ...
    // The prerequisite query must be running before the reaction subscribes.
    ...
    // Build router with solutions directory and test plugins
    let plugin_registry = create_test_plugin_registry();
