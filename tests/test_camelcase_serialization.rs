...
fn test_mock_source_nested_fields_are_camelcase() {
    ...
    assert!(
        json_value.get("dataType").is_some(),
        "dataType should exist"
    );
    assert!(
        json_value.get("intervalMs").is_some(),
        "intervalMs should exist"
    );
    assert!(
        json_value.get("data_type").is_none(),
        "data_type should NOT exist"
    );
    assert!(
        json_value.get("interval_ms").is_none(),
...
fn test_postgres_source_nested_fields_are_camelcase() {
    ...
    assert!(
        json_value.get("slotName").is_some(),
        "slotName should exist"
    );
    assert!(
        json_value.get("publicationName").is_some(),
        "publicationName should exist"
    );
    assert!(
        json_value.get("tableKeys").is_some(),
        "tableKeys should exist"
    );
    assert!(json_value.get("sslMode").is_some(), "sslMode should exist");

    assert!(
        json_value.get("slot_name").is_none(),
        "slot_name should NOT exist"
    );
    assert!(
        json_value.get("publication_name").is_none(),
        "publication_name should NOT exist"
    );
    assert!(
        json_value.get("table_keys").is_none(),
        "table_keys should NOT exist"
    );
    assert!(
        json_value.get("ssl_mode").is_none(),
