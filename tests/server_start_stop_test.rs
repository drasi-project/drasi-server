...
async fn test_auto_start_components() -> Result<()> {
    use drasi_server::{SourceConfig, QueryConfig, ReactionConfig};
...
async fn test_manual_vs_auto_start_components() -> Result<()> {
    use drasi_server::{SourceConfig, QueryConfig};
    use std::collections::HashMap;
    ...
    assert_eq!(
        core.source_manager().get_source_status("auto-source".to_string()).await?,
        ComponentStatus::Running
    );
    assert_eq!(
        core.query_manager().get_query_status("auto-query".to_string()).await?,
        ComponentStatus::Running
    );
    
    // Manual components should still be stopped
    assert_eq!(
        core.source_manager().get_source_status("manual-source".to_string()).await?,
        ComponentStatus::Stopped
    );
    assert_eq!(
        core.query_manager().get_query_status("manual-query".to_string()).await?,
        ComponentStatus::Stopped
    );
    
    // Manually start the manual source
    core.source_manager().start_source("manual-source".to_string()).await?;
    sleep(Duration::from_millis(100)).await;
...
async fn test_component_startup_sequence() -> Result<()> {
    ...
    assert_eq!(
        core.source_manager().get_source_status("source1".to_string()).await?,
        ComponentStatus::Running
    );
    assert_eq!(
        core.source_manager().get_source_status("source2".to_string()).await?,
        ComponentStatus::Running
    );
    assert_eq!(
        core.query_manager().get_query_status("query1".to_string()).await?,
        ComponentStatus::Running
    );
    assert_eq!(
        core.query_manager().get_query_status("query2".to_string()).await?,
        ComponentStatus::Running
    );
    assert_eq!(
        core.reaction_manager().get_reaction_status("reaction1".to_string()).await?,
        ComponentStatus::Running
    );
    assert_eq!(
        core.reaction_manager().get_reaction_status("reaction2".to_string()).await?,
        ComponentStatus::Running
    );
