...
async fn test_source_state_transitions() {
    ...
    let source = source_manager
        .get_source("state-source".to_string())
        .await
        .unwrap();
    ...
    source_manager
        .start_source("state-source".to_string())
        .await
        .unwrap();
    ...
    source_manager
        .stop_source("state-source".to_string())
        .await
        .unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    let source = source_manager
    ...
    source_manager
        .delete_source("state-source".to_string())
        .await
        .unwrap();
...
async fn test_query_state_transitions() {
    ...
    let query = query_manager
        .get_query("state-query".to_string())
        .await
        .unwrap();
    ...
    query_manager
        .stop_query("state-query".to_string())
        .await
        .unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    let query = query_manager
    ...
    query_manager
        .delete_query("state-query".to_string())
        .await
        .unwrap();
...
async fn test_reaction_state_transitions() {
    ...
    let reaction = reaction_manager
        .get_reaction("state-reaction".to_string())
        .await
        .unwrap();
    ...
    reaction_manager
        .stop_reaction("state-reaction".to_string())
        .await
        .unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    let reaction = reaction_manager
    ...
    reaction_manager
        .delete_reaction("state-reaction".to_string())
        .await
        .unwrap();
...
async fn test_data_router_registration_cleanup() {
    ...
    let _rx1 = data_router
        .add_query_subscription("query1".to_string(), vec!["router-source".to_string()])
        .await;

    let _rx2 = data_router
        .add_query_subscription("query2".to_string(), vec!["router-source".to_string()])
        .await;

    // Verify subscriptions exist (by trying to add another with same ID - should replace)
    let _rx1_new = data_router
        .add_query_subscription("query1".to_string(), vec!["router-source".to_string()])
        .await;
...
async fn test_subscription_router_registration_cleanup() {
    ...
    let _rx1 = subscription_router
        .add_reaction_subscription("reaction1".to_string(), vec!["router-query".to_string()])
        .await;

    let _rx2 = subscription_router
        .add_reaction_subscription("reaction2".to_string(), vec!["router-query".to_string()])
        .await;

    // Remove subscriptions
    subscription_router
        .remove_reaction_subscription("reaction1")
        .await;
    subscription_router
        .remove_reaction_subscription("reaction2")
        .await;

    // Adding new subscriptions should work (verifying cleanup)
    let _rx1_after = subscription_router
        .add_reaction_subscription("reaction1".to_string(), vec!["router-query".to_string()])
        .await;
}
...
async fn test_bootstrap_router_registration() {
    ...
    assert!(source_manager
        .get_source("bootstrap-source".to_string())
        .await
        .is_ok());
    assert!(query_manager
        .get_query("bootstrap-query".to_string())
        .await
        .is_ok());
}
...
async fn test_concurrent_state_operations() {
    ...
    for i in 0..10 {
        let sm = source_manager.clone();
        let handle =
            tokio::spawn(async move { sm.start_source(format!("concurrent-source-{}", i)).await });
        handles.push(handle);
    }
    ...
    for i in 0..10 {
        let source = source_manager
            .get_source(format!("concurrent-source-{}", i))
            .await
            .unwrap();
        assert!(matches!(
            source.status,
    ...
    for i in 0..10 {
        source_manager
            .delete_source(format!("concurrent-source-{}", i))
            .await
            .unwrap();
    }
}
...
async fn test_update_preserves_state() {
    ...
    source_manager
        .start_source("update-source".to_string())
        .await
        .unwrap();
    ...
    // Verify it's running
    let source = source_manager
        .get_source("update-source".to_string())
        .await
        .unwrap();
    assert!(matches!(
        source.status,
    ...
    source_manager
        .update_source("update-source".to_string(), updated_config)
        .await
        .unwrap();
    ...
    assert_eq!(
        source.properties.get("interval_ms").unwrap(),
...
async fn test_error_state_handling() {
    ...
    let result = query_manager
        .start_query("error-query".to_string(), rx)
        .await;
    ...
    if result.is_ok() {
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
        let query = query_manager
            .get_query("error-query".to_string())
            .await
            .unwrap();
        assert!(matches!(
            query.status,
...
async fn test_dependency_ordering() {
    ...
    assert!(source_manager
        .get_source("dep-source".to_string())
        .await
        .is_ok());
    assert!(query_manager
        .get_query("dep-query".to_string())
        .await
        .is_ok());
    assert!(reaction_manager
        .get_reaction("dep-reaction".to_string())
        .await
        .is_ok());
}
...
async fn test_cascading_cleanup() {
    ...
    source_manager
        .start_source("cascade-source".to_string())
        .await
        .unwrap();

    let rx = data_router
        .add_query_subscription(
    ...
    query_manager
        .delete_query("cascade-query".to_string())
        .await
        .unwrap();
    ...
    assert!(query_manager
        .get_query("cascade-query".to_string())
        .await
        .is_err());

    // Source and reaction should still exist
    assert!(source_manager
        .get_source("cascade-source".to_string())
        .await
        .is_ok());
    assert!(reaction_manager
        .get_reaction("cascade-reaction".to_string())
        .await
        .is_ok());
}
