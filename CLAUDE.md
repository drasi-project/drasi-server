...
Trading's `./start-demo.sh` and devcontainer share `scripts/prepare-trading.sh`: verify the exact engine, build the checked-out server and real UI, and install all five reviewed registry plugins including the SSE reaction created later by the app. `scripts/install_plugins.py` reuses the existing locked installer with independent binary-hash checks for Trading, test-only plugins and Getting Started. The approved main integration retains host SDK crates 0.11.0 and library 0.9.1, with main-signed plugins built using SDK crate 0.11.1; both have native ABI 0.13.0. Crate versions and ABI metadata are distinct, and old ABI 0.11 / newer ABI 0.14 artifacts are rejected. Server startup verifies signatures. Only verified matching local-SDK development uses locally built, unsigned plugins. Preparation uses Python 3.11 or later. See `docs/main-runtime-integration.md` for current provenance and `docs/engine-prerequisite.md` / `tests/plugin-pins/README.md` for labeled historical evidence and limits.
