...
Trading's `./start-demo.sh` and devcontainer share `scripts/prepare-trading.sh`: verify the exact engine, build the checked-out server and real UI, and install all five reviewed registry plugins including the SSE reaction created later by the app. `scripts/install_plugins.py` reuses the existing locked installer with independent binary-hash checks for both Trading and the pinned test-only plugin set. Registry mode retains SDK crates 0.10.0 and signed ABI 0.11.0 artifacts; server startup verifies their signatures. Only verified matching local-SDK development uses locally built, unsigned plugins. Plugin preparation uses Python 3.11 or later (standard-library TOML support). See `docs/engine-prerequisite.md` and `tests/plugin-pins/README.md` for exact provenance, blockers and limits.
