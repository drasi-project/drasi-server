...
**Important: `[patch.crates-io]` does NOT rebuild plugins.** Cargo patches only affect compile-time dependency resolution for the server binary. Plugins are separate shared libraries loaded at runtime. An engine/AST/parser-only path patch can retain registry library/SDK/FFI dependencies and their compatible signed plugins; the presence of `../drasi-core` is not evidence of a local SDK or ABI change. `scripts/plugin_origin.py` checks the actual locked Cargo dependency graph. The `build-local-plugins*` targets require that the server's local SDK, host, FFI and library identities match the plugin-build workspace; they reject engine-only registry-SDK configurations. Use these targets for genuine local SDK development, not merely because a core checkout exists.
...
Trading's `./start-demo.sh` and devcontainer share `scripts/prepare-trading.sh`: verify the exact engine, build the checked-out server and real UI, and install all five reviewed registry plugins including the SSE reaction created later by the app. `scripts/install_plugins.py` reuses the existing locked installer with independent binary-hash checks for Trading, test-only plugins and Getting Started. The current main integration retains registry host/plugin/FFI SDK crates 0.11.2, library 0.9.2 and index 0.6.3, with official main-signed SDK 0.11.2 plugins from release `70ca432c`; the native ABI is 0.14.0. Old ABI 0.11 and 0.13 artifacts are rejected. Only engine 0.5.9, AST 0.3.5 and Cypher 0.3.6 consume the temporary exact source pin `211d0f2a79aa2ad0f7cb841937f52013fe95ded6` from drasi-project/drasi-core#810. This is not a released fix, a library codec upgrade, or permission to build that sibling's unused SDK plugins: equal crate versions do not imply equal source origins. Server startup verifies signatures. Only verified matching local-SDK development uses locally built, unsigned plugins. Preparation uses Python 3.11 or later. See `docs/main-runtime-integration.md` for current provenance and `docs/engine-prerequisite.md` / `tests/plugin-pins/README.md` for labeled historical evidence and limits.
