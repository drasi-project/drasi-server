...
[`@drasi/react`](../../dev-tools/react/README.md) package. No Trading frontend,
...
[authentication and transport reference](../../dev-tools/react/README.md#authentication-and-injected-transports).
...
In another terminal under `examples/react`, use the **actual printed URL**:
...
cd /tmp/drasi-packed-consumer/examples/react
...
```sh
# In examples/react, after the explicit build/install steps:
npm run typecheck
npm test
npm run build
...
[screen-reader checklist](../trading/TESTING.md#manual-screen-reader-checklist-pending)
...
[`npm run test:browser:linux`](../trading/TESTING.md#reproducible-visual-and-packed-consumer-gate)
...
- `src/table.tsx`: one subscription per table, typed `DataTable` and pure
  `queryTableState`; the package reference also includes the shorter composed
  `QueryTable` recipe.
...
wrappers or implicit instance selection from Trading. The single complete
[package reference](../../dev-tools/react/README.md) covers all public
props/options/defaults, sorts/slots/actions, auth/transports, compatibility,
CSS tokens/portals, migrations and troubleshooting. Do not fork a second API
reference here. These examples share the repository's Apache-2.0 license;
