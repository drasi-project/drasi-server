...
assert.equal(typeof require('@drasi/react').DrasiClient, 'function');
const esm = await import(pathToFileURL(join(app, 'node_modules/@drasi/react/dist/index.js')));
assert.equal(typeof esm.QueryTable, 'function');
assert((await readFile(require.resolve('@drasi/react/styles.css'), 'utf8')).includes('.drasi-query-table'));
