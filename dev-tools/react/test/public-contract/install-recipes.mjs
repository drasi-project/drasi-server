...
export function extractInstallRecipes(markdown) {
  ...
  for (const match of markdown.matchAll(pattern)) {
    ...
    recipes.set(name, { name, variable, args: args.slice(1), code });
  }
...
export async function checkInstallRecipes({ artifact, packageRoot, output, readme }) {
  ...
  try {
    for (const recipe of recipes.values()) {
      ...
      proof.recipes[recipe.name] = { literal: recipe.code, npmArgs: args, renders, builtFilesIdentical: true };
    }
    ...
    run('npm', ['install', '--ignore-scripts', '--install-links=false', '--no-audit', '--no-fund'], unsupported);
    ...
    console.log(`Fresh README installs: copied-local and tarball render ESM/CJS with one React; ${Object.keys(original).length} built files unchanged; unsupported symlink reproduces the real hook failure.`);
    return proof;
  } finally {
