...
//! PostgreSQL stored procedure reaction configuration mapper.

use crate::api::mappings::{ConfigMapper, DtoMapper, MappingError};
use crate::api::models::{
    PostgresStoredProcReactionConfigDto, StoredProcQueryConfigDto, StoredProcTemplateSpecDto,
};
use drasi_lib::reactions::common::{QueryConfig, TemplateSpec};
use drasi_reaction_storedproc_postgres::PostgresStoredProcReactionConfig;
use std::collections::HashMap;

pub struct PostgresStoredProcReactionConfigMapper;

impl ConfigMapper<PostgresStoredProcReactionConfigDto, PostgresStoredProcReactionConfig>
    for PostgresStoredProcReactionConfigMapper
{
    fn map(
        &self,
        dto: &PostgresStoredProcReactionConfigDto,
        resolver: &DtoMapper,
    ) -> Result<PostgresStoredProcReactionConfig, MappingError> {
        ...
        Ok(PostgresStoredProcReactionConfig {
            hostname,
            port,
            user,
            password,
...
mod tests {
    ...
    #[test]
    fn test_postgres_storedproc_mapper() {
        let dto = PostgresStoredProcReactionConfigDto {
            hostname: ConfigValue::Static("localhost".to_string()),
            port: Some(ConfigValue::Static(5432)),
            database: ConfigValue::Static("testdb".to_string()),
            ssl: false,
            identity_provider_id: None,
            user: Some(ConfigValue::Static("testuser".to_string())),
            password: Some(ConfigValue::Static("testpass".to_string())),
            routes: HashMap::new(),
            default_template: None,
            command_timeout_ms: ConfigValue::Static(5000),
            retry_attempts: ConfigValue::Static(3),
        };

        let mapper = PostgresStoredProcReactionConfigMapper;
        let dto_mapper = DtoMapper::new();
        let result = mapper.map(&dto, &dto_mapper).unwrap();
        ...
        assert_eq!(result.command_timeout_ms, 5000);
